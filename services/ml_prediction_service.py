...
def generate_stacking_ensemble(
    ...
    """
    Stacking Ensemble: loads pre-computed Ridge meta-model predictions from disk,
    same format as every other ML model.
    """
    actual_df = _load_actual(category_name, station_name)
    ...
    # Load pre-computed ensemble forecast
    ensemble_forecast = _load_future(station_name, category_name, 'StackingEnsemble', horizon)
    if ensemble_forecast is None:
        raise ValueError(
            f"No pre-computed StackingEnsemble data for {station_name} / {category_name}. "
            "Run generate_stacking_ensemble.py to create the prediction files."
        )

    ensemble_fit_df, ensemble_rmse, ensemble_mape = _load_historical_fit(
        station_name, category_name, 'StackingEnsemble', horizon, actual_df
    )

    # Load base model forecasts for overlay display
    ...
    for model_name in base_models:
        ...
        _, rmse, _ = _load_historical_fit(station_name, category_name, model_name, horizon, actual_df)
        if future_vals is not None:
            base_predictions[model_name] = future_vals
            model_rmses[model_name] = float(rmse) if not np.isnan(rmse) else 100

    # Min/max band across base models
    if base_predictions:
        all_preds = np.array(list(base_predictions.values()))
        forecast_min = np.min(all_preds, axis=0)
        forecast_max = np.max(all_preds, axis=0)
    else:
        forecast_min = ensemble_forecast
        forecast_max = ensemble_forecast
    ...
    # Uncertainty band (range across base models)
    fig.add_trace(go.Scatter(
    ...
    model_colors = {
        'LSTM': '#34d399', 'GRU': '#a78bfa', 'PatchTST': '#f59e0b',
        'DLinear': '#f87171', 'iTransformer': '#60a5fa',
    }
    model_symbols = {
        'LSTM': 'circle', 'GRU': 'square', 'PatchTST': 'diamond',
        'DLinear': 'cross', 'iTransformer': 'triangle-up',
    }

    for model_name, preds in base_predictions.items():
        rmse_label = f'RMSE: {model_rmses[model_name]:.1f}' if model_name in model_rmses else ''
        fig.add_trace(go.Scatter(
            x=connected_dates, y=[last_value] + preds.tolist(),
            mode='lines+markers',
            name=f'{model_name} ({rmse_label})',
    ...
    ensemble_rmse_str = f'{ensemble_rmse:.2f}' if not np.isnan(ensemble_rmse) else 'N/A'
    ensemble_mape_str = f'{ensemble_mape:.1f}%' if not np.isnan(ensemble_mape) else 'N/A'

    return {
        'chart': json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder),
        'model_info': {
            'model': 'Stacking Ensemble (Ridge)',
            'base_models': ', '.join(base_predictions.keys()) if base_predictions else ', '.join(base_models),
            'horizon_days': horizon,
            'rmse': ensemble_rmse_str,
            'mape': ensemble_mape_str,
            'last_historical': last_date.strftime('%Y-%m-%d'),
            'forecast_end': future_dates[-1].strftime('%Y-%m-%d'),
